...
export function CategoriesSection() {
  return (
    <section id="categorias" className="py-12 sm:py-24 lg:py-28 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
          {CATEGORIES_DATA.map((cat) => (
            <div
              key={cat.id}
              className="group relative h-[260px] sm:h-[440px] lg:h-[480px] rounded-2xl sm:rounded-3xl overflow-hidden shadow-luxury-md border border-luxury-sand/50 dark:border-luxury-charcoal-light/50 flex flex-col justify-end p-5 sm:p-8 transition-transform duration-500 hover:-translate-y-1.5 hover:shadow-2xl"
            >
              {/* Background Image */}
              <Image
                src={cat.image}
              ...
              <div className="relative z-10 space-y-1.5 sm:space-y-3">
                <span className="text-[10px] sm:text-[11px] uppercase tracking-[0.2em] sm:tracking-[0.25em] text-luxury-gold-shimmer font-medium font-sans block">
                  {cat.count}
                </span>
                ...
                <p className="text-[11px] sm:text-sm text-luxury-sand/90 font-light line-clamp-1 sm:line-clamp-2 leading-relaxed">
                  {cat.subtitle}
                </p>
                ...
                <div className="pt-2 sm:pt-3 flex items-center justify-between">
                  <a
                    href={`#coleccion`}
                    className="inline-flex items-center space-x-1.5 sm:space-x-2 text-[11px] sm:text-xs uppercase tracking-[0.16em] sm:tracking-[0.2em] font-medium text-white group-hover:text-luxury-gold transition-colors"
                  >
                  ...
                  <a
                    ...
                    className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-luxury-gold/20 hover:bg-luxury-gold text-luxury-gold hover:text-luxury-charcoal-black flex items-center justify-center backdrop-blur-md border border-luxury-gold/40 transition-all duration-300"
                  >
